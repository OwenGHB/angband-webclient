var TU = function(socket) {
    return {
        ...
        transformUI: function() {
            ...
            // move some tabs to common panel
            const mainPanel = $(".tab-panels.uchel");
            $("#tab-chat").appendTo(mainPanel);
            $("#tab-people").appendTo(mainPanel)

            // move respective tab buttons to upper button bar
            const mainBtnPanel = $(".tab-buttons.uchel");
            $('.tab-buttons.isaf a').each(function (i, el) {
                $(el).appendTo(mainBtnPanel);
            });

            // mutate showTab function
            showTab = function (id, context) {
                // deal with button selection
            ...
            // goto tabletui tab
            showTab("tab-tabletui", $("#btn-tabletui"));

            // and remove that button that toggled tablet UI, who needs it now, right?
            $("#tablet-ui-toggle").remove();

            // finally make common panel full height
            $(mainPanel).css("flex-grow", 1);
            $(mainPanel).css("height", "");

            // and remove second panel
            $(".tab-buttons.isaf").remove();
            $(".tab-panels.isaf").remove();
        },
